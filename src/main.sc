...
theme: /
    ...
    state: Discounts
        ...
        intent: /ok || toState = "/Farewell"
        intent: /детали || toState = "/Redirect"
        intent: /акции и льготы/документы || toState = "/Docs"
        event: noMatch || toState = "/NoMatch"
    ...
    state: Docs
        a: На кассе в кинотеатре необходимо предоставить оригиналы документов, дающих льготы: студенческий билет / справку об обучении в школе / пенсионное удостоверение / удостоверение ветерана.
        intent: /ok || toState = "/Farewell"
        event: noMatch || toState = "/NoMatch"